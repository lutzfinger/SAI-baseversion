...
def test_primary_counts():
    assert _is_operator_sender("Lutz Finger <hello@example.com>", OP, SAI)


def test_sai_alias_counts():
    # the bug: a reply the operator's client sent from sai@ used to be dropped
    assert _is_operator_sender("<sai@example.com>", OP, SAI)
...
def test_no_alias_configured_still_matches_primary():
    assert _is_operator_sender("<hello@example.com>", OP, "")
    assert not _is_operator_sender("<sai@example.com>", OP, "")
