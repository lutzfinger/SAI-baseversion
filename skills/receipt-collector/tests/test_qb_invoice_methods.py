    ...
    def fake_request(method, url, **kw):
        ...
        if url.endswith("/customer"):
            return FakeResp(200, {"Customer": {"Id": "C1",
                                  "PrimaryEmailAddr": {"Address": "greg.sample@example.edu"}}})
...
def test_create_customer(qb):
    out = qb.create_customer("Greg Sample", email="greg.sample@example.edu")
    assert out["Id"] == "C1"
    post = [c for c in qb._calls if c["url"].endswith("/customer")][-1]
    body = json.loads(post["data"])
    assert body["DisplayName"] == "Greg Sample"
    assert body["PrimaryEmailAddr"]["Address"] == "greg.sample@example.edu"


def test_send_invoice_sets_cc_then_sends(qb):
    qb.send_invoice("2307", "greg.sample@example.edu", cc_email="hello@example.com")
    ...
    assert upd_body["BillEmailCc"]["Address"] == "hello@example.com"
    send_params = qb._calls[send_idx]["params"]
    assert send_params.get("sendTo") == "greg.sample@example.edu"
