...
def test_llm_tier_pass_and_fail() -> None:
    common = dict(
        request_text="draft a reply to jane about my latest forbes article",
        recipient_email="jane.doe@example.com",
        draft_body="Hi Jane, my latest piece covers X. <url>",
        candidate_emails=["jane.doe@example.com"],
...
_REQUEST = "draft a Gmail reply to jane about my latest Forbes article"
    ...
    def loop(*, system_prompt, user_text, overlay, mode, use_web_search, model=None):
        ...
        if "creating ONE Gmail DRAFT reply" in system_prompt:
            return _ns(json.dumps({
                ...
                "body": "Hi Jane, my latest Forbes piece covers X. https://www.forbes.com/x",
            }))
...
def _patch_daemon_io(monkeypatch):
    ...
    monkeypatch.setattr(ad, "gmail_search", lambda *a, **k: [
        {"from_email": "jane.doe@example.com", "from_name": "Jane Doe",
...
def test_integration_grounded_proceeds(_patch_daemon_io) -> None:
    ...
    result = ad.auto_execute_ad_hoc(
        text=_REQUEST, overlay={},
        claude_loop_fn=_make_loop("jane.doe@example.com", critique_verdict="passed"),
    )
    assert result["did_write"] is True
    assert result["status_label"] == "SAI/plan"
    assert len(created) == 1
    assert created[0]["to"] == "jane.doe@example.com"
