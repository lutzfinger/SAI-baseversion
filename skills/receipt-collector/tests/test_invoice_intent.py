...
def _greg_inbox():
    return FakeGmail([{
        ...
        "payload": {"headers": [
            {"name": "From", "value": "Greg Sample <greg.sample@example.edu>"}]},
    }])
...
def test_turn1_drafts_unsent_and_asks_approval(tmp_state):
    ...
    out = invoice_intent.handle_new_invoice_trigger(
        _greg_inbox(), {"email": {"operator_email": "hello@example.com"}},
        {"id": "m1", "threadId": "t1"},
...
def _open_awaiting(tmp_path):
    intent = email_intents.open_intent(
        thread_id="t1", operator_email="hello@example.com",
    ...
    store.record_pending({"invoice_id": "2307", "trigger_hash": "h",
                          ...
                          "customer": "Greg Sample",
                          "email": "greg.sample@example.edu", "amount": "1500",
                          "services": "keynote", "cc": "hello@example.com"})
    return intent
...
def test_reply_approve_sends_with_cc(tmp_state):
    ...
    assert sends and sends[0][1] == "2307" and sends[0][2] == "greg.sample@example.edu"
    assert sends[0][3] == "hello@example.com"   # CC
...
def test_route_reply_wires_invoice_intent(tmp_state, monkeypatch):
    ...
    intent = email_intents.open_intent(
        thread_id="t9", operator_email="hello@example.com",
