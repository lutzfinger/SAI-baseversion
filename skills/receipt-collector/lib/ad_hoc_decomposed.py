...
reviewable write (e.g. "draft a reply to karin about my latest
Forbes article").
...
Canonical example — request: "draft a Gmail reply to karin about my
latest Forbes article, 3-sentence summary with the link":

{
  "auto_steps": [
    {"step_id": "step_1_find_karin", "kind": "gmail_search", "args": {"query": "from:karin OR karin", "max_results": 5}},
    {"step_id": "step_2_forbes", "kind": "forbes_latest", "args": {"n": 3}}
  ],
  "write_steps": [
    {"step_id": "step_3_draft", "kind": "gmail_create_draft", "proposed_action": "Draft a 3-sentence reply to the chosen Karin about the latest Forbes article, with the link."}
  ],
  "final_response_template": "I don't have a pre-approved skill for this, but I did the read-only parts already.\\n\\nAUTO-EXECUTED (read-only, no cost):\\n\\n{{evidence:step_1_find_karin}}\\n\\n{{evidence:step_2_forbes}}\\n\\nNEEDS YOUR APPROVAL:\\n\\n{{write_step:step_3_draft}}\\n\\nReply 'y' to create the draft, or 'use karin <email>' / 'use article <N>' to steer. Nothing is sent without your review."
}
"""
...
def extract_email(text: str) -> Optional[str]:
    ...
    "no not to this karin but karinbdohm@web.de" -> "karinbdohm@web.de".
...
Resolve:
  - recipient_email: the address to draft to. If steering names an
    ...
    A single clear match (e.g. one "Karin Finger" for "karin") is NOT
...

{
  ...
  "recipient_hint": "<for draft_email: the person to write to, e.g. 'karin'>",
...
def build_and_create_draft(
    *,
    ...
    Karins, no steering), returns the clarification question and creates
...
def auto_execute_ad_hoc(*, text: str, overlay: dict, claude_loop_fn: Callable,
    ...
    # ---- draft_email -------------------------------------------------
    if kind == "draft_email":
        query = (route.get("recipient_hint") or "").strip() or "karin"
        ...
        gmail_q = f"({query}) -from:hello@lutzfinger.com -from:lutztfinger@gmail.com"
        try:
