...
def test_config_loads_ok_with_distance_block():
    ...
    assert "hello@lutzfinger.com" in cfg["operator_addresses"]
...
def test_intake_operator_only():
    ...
    good = intake.parse_trigger_email({"from": "Lutz <hello@lutzfinger.com>", "subject": "yesterday I went to Berkeley"}, operator_addresses=op)
    ...
    assert intake.parse_trigger_email({"from": "hello@lutzfinger.com", "subject": "FW: invoice"}, operator_addresses=op) is None
...
_CFG = {"max_writes_per_day": 5, "operator_addresses": ["hello@lutzfinger.com"], "home_label": "Mountain View",
...
def test_run_once_happy_writes_and_replies():
    gmail = _FakeGmail(["t1"], {"t1": {"from": "hello@lutzfinger.com", "subject": "yesterday I went to Berkeley"}})
...
def test_run_once_flight_needs_human():
    ...
    gmail = _FakeGmail(["t1"], {"t1": {"from": "hello@lutzfinger.com", "subject": "yesterday I went to New York"}})
    replies, ws = [], runner._CaptureWS()
...
def test_run_once_per_day_cap():
    gmail = _FakeGmail(["t1"], {"t1": {"from": "hello@lutzfinger.com", "subject": "yesterday I went to Berkeley"}}, processed_today=5)
    res = run_daemon.run_once(gmail=gmail, fetch_day_context=_fetch_berkeley, config=_CFG,
